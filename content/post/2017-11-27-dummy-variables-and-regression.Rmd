...
assoction bettween GDP and religiosity).
...
This dataset deals with the aforementioned assocation:
...
Basically, a linear model only understands numerical data as predictor(s). Simply put, on the X-axis there must always be numeric data. A dummy variable can be seen as numeric - possessing only two distinct values, $0$ und $1$!. That's why dummy variables work for regression. By the way, many models think this way.  So we are in the position to run our regression right away:
...
We are bettter to take out one region, define it as baseline or comparison region, and compare each region against that region. If all the remaining region variables are at $0$ that means that we are looking at this baseline region.
...
For visualization and many typical reports, it is neccessary to have on column `Region`.
...
Let's sort the boxplots and flipt the axes:
