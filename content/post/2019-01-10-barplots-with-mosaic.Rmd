...
`mosaic` gives tidy data frames using the function `df_stats()`, that works great for numeric summaries, such as `favstats()`. For qualitative variables, things appear more complicated. With two variables, ie., `day ~ sex` things workout, but apparently not for simply `~ day`. What to do in this case`?
