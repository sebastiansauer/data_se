...
Let's make up some data. Tipp: Make up some data conveniently in Excel, copy it to the clipboard, and then paste it as tribble (see below) into R. For the last step, there is this RStudio Addin available "Dataset Loads". Then use the menu item "Paste as Tribble".
...
In order to convince ggplot to plot the qualitative categories of `country` in the right order, better change them to tye factor:
...
We `rev()`erse the levels, so that the *last* category is a the intercept, and the first category is at the end of the axis, ie., on top of the diagram. See the diagram to better undersand this cryptic explanation.
