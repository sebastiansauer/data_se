...
Having written a larger R project is may be of interest which packages have been used. As I did not find a read-to-use package, a colleage of mine - Norman Markgraf - came up with a nice solution. In this post, I build on his solution to provide a function that suits my needs of todays:
...
Now, let's build a funtion that parses all libraries (as explained above):
...
Finally, let's create a text that cites all these packages as in-text citations so that the functions will appear in the bibliogrphay. We will use the pandoc citation format, which is "@id".
...
We (or I at the least) can use this string to cite all the R packages used, somthing like this "The following R packages were used in this project: `format_cite_string(libs_in_project)`".
