...
```{r eval = FALSE}
text <- read_csv(file = fileurl, locale = locale(encoding = "ISO-8859-1"))
```
...
```{r eval = FALSE}
write_csv(x = text, path = "path/to/file.csv")
```
...
```{r eval = FALSE}
write.csv(x = text, file = "test2.csv", fileEncoding = "utf8")
```
...
```{r eval = FALSE}
textxlsxurl <- download.file("https://data-se.netlify.com/download/text.xlsx",
                             destfile = "text.xslx")

text_destfile <- paste0(here(), "/text.xlsx")
text_xlsx <- read_xlsx(path =  text_destfile)

text_xlsx <- read_xlsx(path = "https://data-se.netlify.com/download/text.xlsx")
```
