---
title: Simulation Berkson's paradox
...
dag1_p <- ggdag::ggdag(dag1, use_labels = "label")  + theme_dag_blank()
...
# Story and research question

Is intelligenz (IQ) and dilligence (Fleiss, in German) correlated? We'll explore this question, or rather its causal nuances using simulated data.
...
First, we define our causal model.
...
That is, aptitude (Eignung) is a function of intelligence (IQ) and dilligence (Fleiss), where the input variables have the same impact on the outcome variable (aptitude).
...
df = data.frame(
  ...
  Fleiss = Fleiss,
  Eignung = Eignung,
  Studium_bin = ifelse(Eignung > 0, "Studium", "kein Studium")
)
```
...
```{r}
cor(IQ ~ Fleiss, data = df %>% filter(Studium_bin == "Studium"))
...
```{r}
gf_point(IQ ~ Fleiss, data = df %>% filter(Studium_bin == "Studium")) %>% 
